...
export class NZISM36Checks extends NagPack {
  ...
  private checkCloudTrail(node: CfnResource): void {
    ...
    this.applyRule({
      info: 'The trail does not have encryption enabled  - MUST 16.6.12.C.01[CID:2022], MUST 23.5.11.C.01[CID:7496]',
      explanation:
  ...
  private checkELB(node: CfnResource): void {
    ...
    this.applyRule({
      info: 'The CLB does not restrict its listeners to only the SSL and HTTPS protocol | SHOULD 14.5.8.C.01[CID:1667], MUST 16.1.37.C.01[CID:1847], MUST 17.1.48.C.03[CID:2091], SHOULD 22.1.24.C.04[CID:4839]',
      explanation:
