...
describe('Amazon SNS', () => {
  test('NIST.800.53.R4-SNSEncryptedKMS: SNS Topics are encrypted via AWS Key Management Service (KMS)', () => {
    ...
    expect(messages1).toContainEqual(
      expect.objectContaining({
        entry: expect.objectContaining({
          data: expect.stringContaining('NIST.800.53.R4-SNSEncryptedKMS:'),
        }),
      })
    );
