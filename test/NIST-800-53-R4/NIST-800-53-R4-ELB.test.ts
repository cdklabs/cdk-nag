...
describe('Elastic Load Balancing', () => {
  ...
  test('NIST.800.53.R4-ALBWAFEnabled: - ALBs are associated with AWS WAFv2 web ACLs - (Control IDs: SC-7, SI-4(a)(b)(c))', () => {
    ...
    expect(messages).toContainEqual(
      expect.objectContaining({
        entry: expect.objectContaining({
          data: expect.stringContaining('NIST.800.53.R4-ALBWAFEnabled:'),
        }),
      })
    );
